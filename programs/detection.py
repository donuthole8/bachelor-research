...
#   _dis = cv2.imread('results/tex/dissimilarity.png', cv2.IMREAD_COLOR)
  ...
  for l in range(1,label):
    ...
    _landslide = (Lp<180)&(ap>128)&(sp>50)
    landslide = _landslide

    _flooded = (Lp>135)&(ap>128)&(sp<=80)&(ep<0.2)
    flooded = _flooded

    # duplicate = _flooded&_landslide
    ...
    if (np.count_nonzero(flooded)>np.count_nonzero(~flooded)):
      ...
      _fld[idx],_det[idx] = 0,0
    if (np.count_nonzero(landslide)>np.count_nonzero(~landslide))&(np.count_nonzero(flooded)>np.count_nonzero(~flooded)):
      if (ep>0.3):
        bl[idx],gl[idx],rl[idx] = (bo[idx]*al+100*(1-al)),(go[idx]*al+70*(1-al)),(ro[idx]*al+230*(1-al))
        b[idx],g[idx],r[idx] = (bo[idx]*al+100*(1-al)),(go[idx]*al+70*(1-al)),(ro[idx]*al+230*(1-al))
        _lnd[idx],_det[idx] = 0,0
  ...
  cv2.imwrite('results/detection.png', det)
...
def rejection(org,img):
  ...
  lab,hsv = cv2.cvtColor(img,cv2.COLOR_BGR2Lab),cv2.cvtColor(img,cv2.COLOR_BGR2HSV)
  _dis = cv2.imread('results/tex/dissimilarity.png', cv2.IMREAD_COLOR)
  _edge = cv2.imread('results/edge/sobel.png', cv2.IMREAD_COLOR)
